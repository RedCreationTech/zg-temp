...
高严重度异常会直接按 P1 进入总部升级态, 不需要等待人工再次点击“升级”.

如果用户点击“关闭”, 但底层触发条件仍然存在, 例如:
...
Control Tower 会在下一次渲染时自动重新打开该异常.
...
只有底层原因消失以后, 异常才真正退出 Exception Center.
...
实际进度来自真实任务勾选状态，不是固定数字。

### Owner Progress

所有 30 / 60 / 90 天任务会按 Owner 汇总：
...
如果模拟执行日已经超过该阶段计划窗口，而任务仍未完成，系统会自动生成：
...
- 61–90 天任务完成度
- Champion Pattern 是否复现
- Management Closure 是否稳定
- 是否应该继续扩大复制
...
未通过 Day 30 Gate 时，第二批保持：

```text
LOCKED
```

当前候选区域已经补充了真正的第二批医院 / 代表池，因此解锁后会出现新的扩展对象。
